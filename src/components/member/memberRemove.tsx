...
function UserSwitch({ key_list, callback, children, text }: Props) {
  // Redux
        ...
        {memberList.map((member) => (
          ...
          >
            <Checker
              ...
              callback={() => {
                key_list.includes(member) ? null : callback(member)
              }}
            >
              <Avatar key={member} name={users[member].name} src={users[member].avatar} size="xs" mr={2}/>
              <Box className="clipText" w="230px" color={key_list.includes(member) ? "gray" : ""}>{users[member].name}</Box>
            </Checker>
          </Box>
        ))}
      </PopoverBody>
